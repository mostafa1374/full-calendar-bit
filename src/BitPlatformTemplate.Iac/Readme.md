...
Notes:
* `app` is an abbreviation for BitPlatformTemplate, use the acronym of your choice and replace app with that (for example abc) using exact match - case sensitive find and replace in this file and AdStack.cs.
...
pulumi config set BitPlatformTemplate.Iac:sql-server-app-db-admin-id
pulumi config set BitPlatformTemplate.Iac:sql-server-app-db-admin-password --secret

# Provide SMTP server's host, port, user, pass and default email sender.
pulumi config set BitPlatformTemplate.Iac:default-email-from
pulumi config set BitPlatformTemplate.Iac:email-server-host
pulumi config set BitPlatformTemplate.Iac:email-server-port
pulumi config set BitPlatformTemplate.Iac:email-server-userName
pulumi config set BitPlatformTemplate.Iac:email-server-password --secret

# Create and provide an identity certificate password
pulumi config set BitPlatformTemplate.Iac:identity-certificate-password --secret
```
